...
search_words = ['Sazerac','Larue','Stagg','Handy','K&L Exclusive','Gigondas']
...
skus_recently_alerted_on = list(read_alerts(now_aware + datetime.timedelta(minutes = -120))) # don't alert on a product that was alerted on within the last 60m
...
for row in rows: # loop through each row in the table, cut out whitespace for each cell
	...
	if 'Sold Out' not in cols[5]: # TODO: can these exclusion lists be put on one line?
		if not any(b in cols[1] for b in sku_blacklist): # check if have alerted on the SKU in the last 60m.  if so, don't send another alert
			if cols[0] >= now_aware + datetime.timedelta(minutes = -120): # require the product was posted in last 30 mins
